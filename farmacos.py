...
def crear_grafo_plotly(G, farmaco_principal=None, active_categories=None):
    """Crear visualización del grafo con Plotly - VERSIÓN CORREGIDA"""
    ...
    if active_categories is None or not any(active_categories.values()):
        ...
        fig.update_layout(
            title=dict(
                text="No hay fármacos para mostrar<br>Selecciona categorías a la derecha",
                font=dict(size=16),
                x=0.5,
                y=0.5,
                xanchor='center',
    ...
    # PASO 1: Identificar nodos a mantener
    nodes_to_keep = set()
    
    # Siempre incluir el fármaco principal si existe
    if farmaco_principal:
        nodes_to_keep.add(farmaco_principal)
    
    # Agregar nodos según categorías activas
    for node in G.nodes():
    ...
    # PASO 2: Crear subgrafo solo con nodos válidos
    G_filtered = G.subgraph(nodes_to_keep).copy()
    
    # DEBUGGING: Imprimir información
    print(f"DEBUG - Nodos totales en G: {len(G.nodes())}")
    print(f"DEBUG - Nodos a mantener: {len(nodes_to_keep)}")
    print(f"DEBUG - Nodos en G_filtered: {len(G_filtered.nodes())}")
    print(f"DEBUG - Aristas en G_filtered: {len(G_filtered.edges())}")
    
    # Verificar que el grafo filtrado tiene contenido
    if len(G_filtered.nodes()) == 0:
        fig = go.Figure()
        fig.update_layout(
            title=dict(
                text="No hay conexiones para mostrar",
                font=dict(size=16),
                x=0.5,
                y=0.5,
                xanchor='center',
    ...
    # PASO 3: Calcular layout según si hay fármaco principal
    if farmaco_principal and farmaco_principal in G_filtered.nodes():
        # Layout radial centrado en el fármaco principal
        pos = {}
        pos[farmaco_principal] = np.array([0, 0])
        
        # Obtener vecinos (predecesores y sucesores)
        neighbors = list(set(
            list(G_filtered.predecessors(farmaco_principal)) + 
        ...
        pos = nx.spring_layout(
            G_filtered, 
            k=2/np.sqrt(len(G_filtered.nodes())), 
            iterations=50, 
            seed=42
        )
    
    # PASO 4: Preparar datos de nodos
    ...
    for node in G_filtered.nodes():
        ...
        # Tamaño según si es el principal
    ...
    # PASO 5: Preparar datos de aristas
    edge_x, edge_y, edge_texts = [], [], []
    edge_count = 0  # Contador manual
    ...
    for u, v, data in G_filtered.edges(data=True):
        ...
        edge_texts.append(data.get('tooltip', f"{u} → {v}"))
        edge_count += 1
    
    print(f"DEBUG - Aristas contadas manualmente: {edge_count}")
    ...
    # PASO 7: Crear título con conteos CORRECTOS
    num_nodes = len(G_filtered.nodes())
    num_edges = len(G_filtered.edges())
    
    if farmaco_principal:
        # Contar conexiones directas del fármaco principal
        if farmaco_principal in G_filtered.nodes():
            incoming = len(list(G_filtered.predecessors(farmaco_principal)))
            outgoing = len(list(G_filtered.successors(farmaco_principal)))
            total_connections = incoming + outgoing
            
            title_text = (
                f"<b>Fármaco:</b> {farmaco_principal}<br>"
                f"<b>Nodos:</b> {num_nodes} | <b>Interacciones:</b> {num_edges} | "
                f"<b>Conexiones directas:</b> {total_connections} (↓{incoming} ↑{outgoing})"
            )
        else:
            title_text = f"<b>Red Completa</b><br><b>Nodos:</b> {num_nodes} | <b>Interacciones:</b> {num_edges}"
    else:
        title_text = f"<b>Red Completa</b><br><b>Nodos:</b> {num_nodes} | <b>Interacciones:</b> {num_edges}"
